...
public class RelationshipAnalyser {
   ...
   // private final ApplicationProperties props = null;
   // Global Variables
   ...
   public Relationships analyse() throws APIException, IntegrityException {
      ...
      LinkedHashMap<String, IntegrityField> fields = Integrity.getSysFields();
         ...
         if (!field.getAllowedTypes().isEmpty()) {
            for (Object object : field.getIField("allowedTypes").getList()) {
               ...
               for (String toType : fromType.getField("to").getValueAsString().split(",")) {
                  ...
                  cnt++;
                  SimpleWorkItem item = new SimpleWorkItem("CONTENT", Integer.toString(cnt));
   ...
   public Relationships analyseTraces(List<IntegrityObject> fieldList) throws APIException, IntegrityException {
      Relationships rels = new Relationships();
      ...
      IntegrityTypePropertyList tpl = new IntegrityTypePropertyList(session, solutionTypeName, prefix);
      cnt = 0;
      log("\nPart 2: Trace Definitions in Solution Type\n\n" + header2);
